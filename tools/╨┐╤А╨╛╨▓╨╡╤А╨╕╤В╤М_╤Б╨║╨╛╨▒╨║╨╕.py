...
def scan(path):
    ...
    # ---- ОБРУБКИ МНОГОСТРОЧНЫХ ВЫРАЖЕНИЙ ----
    #
    # Строка, начинающаяся с + или : сразу после точки с запятой, означает
    # остаток вырезанного выражения. Так вышло при удалении мёртвого кода: разбор
    # убрал первую строку метода Describe, а продолжение осталось.
    #
    # Пробник этого не ловит: он не собирает 27 файлов из 81.
    raw = s.split("\n")
    for i, line in enumerate(raw):
        t = line.strip()
        if not t or t[0] not in "+:": continue
        if t.startswith("://"): continue
        j = i - 1
        while j >= 0 and not raw[j].strip(): j -= 1
        if j < 0: continue
        prev = raw[j].strip()
        if prev.endswith(";") or prev.startswith("//"):
            return f"обрубок выражения в строке {i+1}: {t[:40]}"
